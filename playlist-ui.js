...
(function () {
    ...
    function ensureSeedPlaylists() {
        ...
        var seeded = {};
        if (baseTracks.length) seeded['Night Drive'] = baseTracks.slice(0, 3);
        if (baseTracks.length) seeded['Chill Mix'] = baseTracks.slice(Math.max(0, baseTracks.length - 3));
        write(STORAGE_KEY, Object.keys(seeded).length ? seeded : { 'Моя колекція': [] });
    }
    ...
    function trackFromRow(row) {
        // API search/trending tracks are kept on the row by index.js. This is
        // important because they are not part of localTracks.
        ...
        var lists = [window.localTracks || [], window.activeTracks || [], window.searchResults || []];
    ...
    function openPicker(track) {
        ...
        picker.innerHTML = '<section class="playlist-picker" role="dialog" aria-modal="true">' +
            '<div class="playlist-picker-left"><input class="playlist-search" type="search" placeholder="Поиск плейлиста"><button class="playlist-create" type="button"><span class="material-symbols-outlined">add</span>Новый плейлист</button><div class="playlist-picker-list"></div></div>' +
    ...
    function openPlaylist(name) {
        ...
        if (!view) { view = document.createElement('div'); view.id = 'playlist-detail-view'; view.className = 'playlist-detail-view'; document.body.appendChild(view); }
        ...
        view.querySelector('.playlist-add-track').addEventListener('click', function () { view.remove(); if (window.openPlaylistPicker && window.localTracks && window.localTracks[0]) window.openPlaylistPicker(window.localTracks[0]); });
    ...
    function playlistMenu(name, anchor) {
        ...
        menu.innerHTML = '<button data-action="edit" type="button">Изменить</button><button data-action="delete" type="button">Удалить</button>';
        menu.querySelector('[data-action="edit"]').addEventListener('click', function () { editPlaylist(name); menu.remove(); });
        menu.querySelector('[data-action="delete"]').addEventListener('click', function () { var data = playlists(); delete data[name]; write(STORAGE_KEY, data); menu.remove(); renderCards(); var view = document.getElementById('playlist-detail-view'); if (view) view.remove(); });
    ...
    function renderCards() {
        ...
        Object.keys(data).forEach(function (name) {
            ...
            card.querySelector('img').src = coverForPlaylist(name, data[name]); card.querySelector('.card-title').textContent = name; card.querySelector('.card-subtitle').textContent = data[name].length + ' трек(ів)';
